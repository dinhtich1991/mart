...
class Lienhe extends CI_Controller {
     ...
     */
    private $Controller = "lienhe";
    private $task;    
    
    public function __construct() {
        parent::__construct();    
        if(!$this->session->userdata('idAdmin')) redirect(PATH_FOLDER_ADMIN.'/login');
        $this->load->model(PATH_FOLDER_ADMIN.'/lienhe_model', 'lienhe');  
    ...
    public function p($page=0){       
        ...
        $config['total_rows']  = $this->lienhe->total_rows();
        $config['per_page']    = ADMIN_PER_PAGE; 
        $config['num_links']   = ADMIN_NUM_LINKS;
        $config['cur_page']    = $page;
        $this->pagination->initialize($config); 
        $data['total_rows']    = $config['total_rows'];
        $data['list']          = $this->lienhe->display($config['per_page'],$page);     
    ...
    public function add(){        
        $data = $this->task;        
        if ($this->input->post()) {
            if($this->lienhe->add()) $this->messages->add(MSG_ADD_SUCCESS, 'success');
        ...
        $this->load->view(PATH_FOLDER_ADMIN.'/view.header.php',$data);
        /* #### */        
        $data['orderingMax']    = $this->lienhe->orderingMax();
    ...
    public function edit($id){
        ...
        $data['detail']         = $this->lienhe->getList((int)$id);       
    ...
    public function status($id=0,$status=0,$field='status'){
        echo $this->lienhe->status($id,$status,$field);
    }
    /**
    ...
    public function search(){
        ...
        $data['list']          = $this->lienhe->displaySearch($filter_name);     
