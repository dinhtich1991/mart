...
class Duyet extends CI_Controller {
    ...
    public function p($page = 0) {
        ...
        $data['title_header'] = "Duyệt dự án";

        #Phân trang
        $config['base_url'] = $data['page'];
        $config['total_rows'] = $this->product->total_rows2();
        $config['per_page'] = ADMIN_PER_PAGE;
        $config['num_links'] = ADMIN_NUM_LINKS;
        $config['cur_page'] = $page;
        $this->pagination->initialize($config);
        $data['total_rows'] = $config['total_rows'];
        $data['list'] = $this->product->display2($config['per_page'], $page);
    ...
    public function add() {
        ...
        $data['title_header'] = "Thêm - Duyệt dự án";
