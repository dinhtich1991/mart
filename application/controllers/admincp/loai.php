...
class Loai extends CI_Controller {
    ...
    public function task() {
        ...
        $data['loai'] = PATH_FOLDER_ADMIN . "/" . $this->Controller . "/p";
        return $data;
    }
    ...
    public function p($loai = 0) {
        $data = $this->task;
        $data['title_header'] = "loai";
        $this->load->view(PATH_FOLDER_ADMIN . '/view.header.php', $data);
        $config['base_url'] = $data['loai'];
        $config['total_rows'] = $this->loai->total_rows();
        $config['per_page'] = ADMIN_PER_PAGE;
        $config['num_links'] = ADMIN_NUM_LINKS;
        $config['cur_page'] = $loai;
        $this->pagination->initialize($config);
        $data['total_rows'] = $config['total_rows'];
        $data['list'] = $this->loai->display($config['per_page'], $loai);
    ...
    public function add() {
        ...
        $data['title_header'] = "Add new - loai";
        $this->load->view(PATH_FOLDER_ADMIN . '/view.header.php', $data);
        $data['orderingMax'] = $this->loai->orderingMax();
    ...
    public function edit($id) {
        ...
        $data['title_header'] = "Edit - loai";
        $this->load->view(PATH_FOLDER_ADMIN . '/view.header.php', $data);
        /* #### */

        $data['detail'] = $this->loai->getList((int) $id);
    ...
    public function status($id = 0, $status = 0, $field = 'status') {
        echo $this->loai->status($id, $status, $field);
    }
