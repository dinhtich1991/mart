...
class Doitac extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model(PATH_FOLDER_ADMIN . '/doitac_model', 'doitac');
    ...
    public function task() {
        ...
        $data['doitac'] = PATH_FOLDER_ADMIN . "/" . $this->Controller . "/p";
        return $data;
    }
    ...
    public function p($doitac = 0) {
        $data = $this->task;
        $data['title_header'] = "Đối tác";
        $this->load->view(PATH_FOLDER_ADMIN . '/view.header.php', $data);
        $config['base_url'] = $data['doitac'];
        $config['total_rows'] = $this->doitac->total_rows();
        $config['per_page'] = ADMIN_PER_PAGE;
        $config['num_links'] = ADMIN_NUM_LINKS;
        $config['cur_page'] = $doitac;
        $this->pagination->initialize($config);
        $data['total_rows'] = $config['total_rows'];
        $data['list'] = $this->doitac->display($config['per_page'], $doitac);
    ...
    public function add() {
        ...
        $data['title_header'] = "Add new - Đối tác";
        $this->load->view(PATH_FOLDER_ADMIN . '/view.header.php', $data);
        $data['orderingMax'] = $this->doitac->orderingMax();
    ...
    public function edit($id) {
        $data = $this->task;
        if ($this->input->post()) {
            $this->doitac->update($id);
            $this->messages->add(MSG_EDIT_SUCCESS, 'success');
            redirect($data['task_list']);
        }

        $data['title_header'] = "Edit - Đối tác";
        $this->load->view(PATH_FOLDER_ADMIN . '/view.header.php', $data);
        /* #### */

        $data['detail'] = $this->doitac->getList((int) $id);
    ...
    public function status($id = 0, $status = 0, $field = 'status') {
        echo $this->doitac->status($id, $status, $field);
    }
